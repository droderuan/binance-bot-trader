...
class UserWalletService {
  ...
  async getInitialBalance() {
    try {
      const balanceResponse = await binanceClient.currentBalance([
        this.PairInfo.buyCoin,
        this.PairInfo.sellCoin,
      ]);
      ...
      logger.log({
        from: "WALLET",
        message: "while updating balance",
        type: "ERROR",
      });
    }
  }
  ...
  async getLastOrder() {
    return this.lastOrder
      ? this.lastOrder
      : binanceClient.lastOrder(this.PairInfo.pair);
  }
